...
**为国内云厂商 DNS 实现的 cert-manager ACME DNS-01 校验 Webhook**

阿里云 · 腾讯云 · 华为云 · 百度智能云 —— 一个二进制,全部搞定
...
cert-manager 未内置国内这几家云厂商的 DNS-01 solver。本项目用**一个可插拔的二进制**补齐了这块空缺:按 `solverName` 切换厂商,各自调用官方且在维护的 SDK,**幂等、并发安全**,并可签发**通配符证书**。
...
> [!WARNING]
> 首次使用请先读文末的 [⚠️ 注意事项](#-注意事项)。两条最常见的坑——**用了 staging 导致证书不受信任**、**证书只认 SAN 不认 CN**——会让你白忙一场。

## 📋 前置条件

- **Kubernetes 集群** —— v1.27+
- **[cert-manager](https://cert-manager.io/docs/installation/) v1.0+** —— chart 使用 `cert-manager.io/v1` API,并依赖其 `cainjector` 注入 CA;建议用受支持的较新版本
- **域名 + 云厂商 API 凭据** —— 域名需托管在下列某家云 DNS 上:
...
> [!TIP]
> - 默认安装最新版本;如需固定版本,追加 `--version <x.y.z>`。
> - chart 默认使用镜像 `ghcr.io/itmisx/cert-manager-webhook`(标签取 chart 的 `appVersion`),无需额外设置。
> - `groupName` 可为任意你能控制的域名,只需与每个 Issuer 的 `webhook.groupName` **保持一致**。
...
> [!NOTE]
> 对 ClusterIssuer 而言,Secret 需放在 **cert-manager 所在命名空间**(默认 `cert-manager`)。
...
`dnsNames` 会成为证书的 SAN(浏览器只认它);DNS-01 还能签发**通配符证书**:
...
<details>
<summary><b>🔵 <code>solverName: tencentcloud</code>(腾讯云 / DNSPod)</b></summary>
...
<details>
<summary><b>🔴 <code>solverName: huaweicloud</code>(华为云)</b></summary>
...
<details>
<summary><b>⚪ <code>solverName: baiducloud</code>(百度智能云)</b></summary>
...
查看已签发证书的**签发者与 SAN**(排查 staging 陷阱最快的方式):
...
<details>
<summary><b>🧪 一致性测试(访问真实 DNS)</b></summary>

<br>

cert-manager 官方 DNS-01 一致性套件会访问**真实 DNS**,用构建 tag 隔离、默认跳过。运行前填好 `testdata/<provider>-solver/config.json` 与含真实凭据的 `secret.yaml`(参考 `testdata/alidns-solver/README.md`),然后:
...
> [!IMPORTANT]
> 下面两条是最容易导致「证书不受信任 / 浏览器不识别」的坑,**强烈建议阅读**。

<details>
<summary><b>坑 1:用了 staging 环境 → 证书不受信任(<code>YR2</code> / <code>CN</code> 的困惑)</b></summary>
...
<details>
<summary><b>坑 2:Common Name vs. SAN → Chrome/Safari 不识别证书</b></summary>

...
type Config struct {
	...
	AccessKeyIDRef     provider.SecretKeySelector `json:"accessKeyIdSecretRef,omitempty"`
	AccessKeySecretRef provider.SecretKeySelector `json:"accessKeySecretSecretRef,omitempty"`
...
func resolveCredentials(ctx context.Context, cfg Config, namespace string, resolve provider.SecretResolver) (keyID, secret string, err error) {
	...
	if keyID == "" || secret == "" {
		return "", "", fmt.Errorf("alidns: access key id and secret must be provided via accessKeyIdSecretRef/accessKeySecretSecretRef")
	}
